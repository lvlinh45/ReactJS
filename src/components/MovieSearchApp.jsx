  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?query=${queryDebounce}&api_key=49f5e2c77c7ca2dce553af90f77b63dd`
      );
  ...
  return (
    <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
      <div className="h-[297px]">
        <LoadingSkelenton height="100%" radius="16px"></LoadingSkelenton>
      </div>
      <div className="p-7 flex flex-1 flex-col">
        <h3 className="text-lg text-black font-semibold mb-4">
          <LoadingSkelenton height="20px"></LoadingSkelenton>
        </h3>
        <p className="text-[#999] text-sm mb-6 !leading-loose">
          <LoadingSkelenton height="10px"></LoadingSkelenton>
...
const MovieItem = ({ data }) => {
  console.log("TCL: MovieItem -> props", data);
  return (
    <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
      <div className="h-[297px]">
        <img
          src={`${imageAPi}${data.backdrop_path}`}
          alt=""
